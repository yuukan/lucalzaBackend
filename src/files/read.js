 ...
 const getEmpresas = async () => {
     try {
         ...
         for (let i = 0; i < empresas.length; i++) {
             ...
             let cuentas = [];
             writeOutput("Sequence	AccountCode	DebitAmount	CreditAmount	DueDate	Memo	Reference", ruta_archivos_bancos);
             writeOutput("Sequence	AccountCode	DebitAmount	CreditAmount	DueDate	Memo	Reference", ruta_archivos_bancos);
             for (let j = 0; j < bancos.length; j++) {
                 let path = ruta_archivos_bancos + bancos[j].BankCode + '/';
 
                 // Truncate the output file
                 fs.truncate(ruta_archivos_bancos + '/output/' + bancos[j].BankCode + '.txt', 0, function () {
